...
import { CFG, applyEnd, oreBreakdown, runAI, score } from "@/lib/mining-engine";
...
export async function POST(
  ...
  return NextResponse.json({
    ...
    you,
    ai,
    // Ore / units / derived units (stockpile preview) / sell price — same
    // formula settleRun() actually commits if the player picks "Stockpile
    // ore", computed once here so the preview and the real thing can't
    // disagree (see oreBreakdown() in lib/mining-engine.ts).
    breakdown: oreBreakdown(r.s.banked, oreData),
    seed: r.s.seed,
