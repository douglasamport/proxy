...
export const CFG = {
  ...
  DIG_FUEL: 0.9, // surcharge for cutting into fresh rock
  TUNNEL_MULT: 0.4, // cost multiplier for driving an existing tunnel
  ...
  ORE_UNITS_MIN: 2,
  ORE_UNITS_MAX: 7,
  TIER_GATE: [0, 0, 0.14, 0.3, 0.46], // pocket distance gates its grade
  ...
  SURVEY: {
    none: {
      ...
      blur: 0,
      label: "None",
      note: "Go in blind. Cheapest, and the only way to be surprised.",
    },
    basic: {
      cost: 50,
      blur: 3.2,
      label: "Basic",
      note: "Pocket count and mass. Positions very rough.",
    },
    full: {
      cost: 120,
      blur: 1.7,
...
/* ============================================================================
   ...
   Portable straight into a Node service.
   ========================================================================== */
...
export function generateField(seed: number): {
  ...
  const pockets: Pocket[] = scatter(
    rng,
    per(CFG.DEPOSITS_PER_100),
    base,
    2.4,
  ).map((p) => {
  ...
  for (let y = 0; y < CFG.H; y++) {
    for (let x = 0; x < CFG.W; x++) {
      ...
      if (cav && rng() < 1 - (cav.d / CFG.CAVERN_SPREAD) * 0.75) {
        c.cavern = true;
        c.dug = true;
      }
      c.known = false;
      ...
      if (!c.cavern && sm && rng() < 1 - (sm.d / CFG.SEAM_SPREAD) * 0.55) {
        c.seam = true;
      }
...
export function applySurvey(
  ...
  if (!plotRoute) {
    s.log.push({
      n: ++s.step,
      t: `${spec.label.toLowerCase()} survey filed`,
      c: 0,
      k: "ping",
    });
    return s;
  ...
  s.log.push({
    n: ++s.step,
    t: `${spec.label.toLowerCase()} survey · ${s.contacts.length} pockets`,
    c: 0,
    k: "ping",
  });
  return s;
...
function pathCost(
  ...
  tx: number,
  ty: number,
  startDir?: DirKey | null,
): number {
  let cost = 0,
    cx = fx,
    cy = fy,
    dir: DirKey | null = startDir ?? null,
    guard = 0;
...
export const DIRS: Record<DirKey, [number, number]> = {
  N: [0, -1],
  S: [0, 1],
  E: [1, 0],
  W: [-1, 0],
...
export function applyMove(state: RunState, dirKey: DirKey): ApplyResult {
  ...
  cell.known = true;
  for (const k of Object.keys(DIRS) as DirKey[]) {
    // you can see the faces around you
    const ax = nx + DIRS[k][0],
      ay = ny + DIRS[k][1];
...
export function applyPing(state: RunState): ApplyResult {
  const s = clone(state);
  if (s.status !== "active") return { s, err: "run over" };
  if (s.step < s.pingReady)
    return { s, err: `sensors recharging (${s.pingReady - s.step} moves)` };
  ...
  for (const g of groups) {
    ...
    const ox = g.cx + (rng() * 2 - 1) * blur,
      oy = g.cy + (rng() * 2 - 1) * blur;
    if (ex) {
  ...
  s.log.push({
    n: ++s.step,
    t: `ping · ${found} contact${found === 1 ? "" : "s"}${s.bearing ? " + bearing" : ""}`,
    c: cost,
    k: "ping",
  });
  return { s };
...
const pruneContacts = (s: RunState) => {
  s.contacts = s.contacts.filter((k) => !contactResolved(s, k));
};
...
export function applyEnd(state: RunState, force?: boolean): ApplyResult {
  const s = clone(state);
  if (!atBase(s) && !force) {
    return { s, err: "must be at base" };
  }
  if (!atBase(s)) {
    s.status = "stranded";
    s.log.push({ n: ++s.step, t: "abandoned in field", c: 0, k: "bad" });
    return { s };
  }
  unloadInto(s);
  s.status = "banked";
...
export function runAI(
  ...
  let goal: { x: number; y: number; kind: "ore" | "contact" | "probe" } | null =
    null;
  const goalDead = (st: RunState) =>
    !goal ||
  ...
  while (s.status === "active" && guard++ < 6000) {
    ...
    if (!goalDead(s)) {
      ...
      if (idx(next.x, next.y) !== before || next.status !== "active") {
        s = next;
        continue;
      }
    ...
    if (!atBase(s) && s.fuel < returnCost(s, s.x, s.y) * 1.2 + 3) {
      s = stepHome(s);
      continue;
    }
    ...
    if (
      !target &&
      s.step >= s.pingReady &&
      s.fuel > chassis.pingFuel + returnCost(s, s.x, s.y) * 1.5
    ) {
      const r = applyPing(s);
      if (!r.err) {
        s = r.s;
    ...
    if (target) {
      goal = { x: target.x, y: target.y, kind: "ore" };
...
function pickTarget(s: RunState, chassis: Chassis): Cell | null {
  ...
  for (const c of s.cells) {
    ...
    if (
      c.known &&
      c.hazard > 0 &&
      (thin || s.sink - c.hazard <= chassis.sinkCap * 0.3)
    )
      continue;
...
function stepToward(s: RunState, tx: number, ty: number): RunState {
  ...
  let bestDir: DirKey | null = null,
    bestVal = Infinity;
  for (const k of Object.keys(DIRS) as DirKey[]) {
    const nx = s.x + DIRS[k][0],
      ny = s.y + DIRS[k][1];
    if (!inBounds(nx, ny)) continue;
    const c = cellAt(s, nx, ny);
    if (c.seam) continue;
