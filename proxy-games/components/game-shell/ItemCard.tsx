  ...
  return (
    <div className="relative pl-6 pt-6">
      ...
      >
        ...
        >
          <p className={`font-mono text-xl font-bold ${a.text}`}>
            {cost.toLocaleString()}
          </p>
          <span
            className={`text-[10px] font-normal uppercase tracking-[.2em] ${ATOMS.textDim}`}
          >
            cr
          </span>
        </div>
        ...
        {/* actions */}
        <div
          className={`flex w-44 shrink-0 flex-col justify-center gap-2 border-l ${ATOMS.borderLine} p-4`}
        >
          <button
            onClick={onBuy}
            disabled={disabled}
            className={`rounded px-4 py-2 text-[11px] font-bold uppercase tracking-[.14em] ${ATOMS.textVoid} transition ${a.btn} ${SURFACE.btnDisabled}`}
          >
            {owned
              ? "Fitted"
              : busy
          ...
          {canSell && (
            <button
              ...
              disabled={sellDisabled}
              title={`${sellValue!.toLocaleString()} cr each · ${sellQuantity} available`}
              className={`rounded border ${ATOMS.borderLine} px-4 py-2 text-[11px] font-bold uppercase tracking-[.14em] ${ATOMS.textDim} transition hover:bg-white/5 ${SURFACE.btnDisabled}`}
            >
              {sellBusy ? "…" : `Sell`}
            </button>
          )}
